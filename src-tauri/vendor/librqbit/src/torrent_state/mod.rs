...
pub struct ManagedTorrentShared {
    pub id: TorrentId,
    pub info_hash: Id20,
    /// LOCAL PATCH: notified to make the tracker loops announce immediately.
    pub reannounce: Arc<tokio::sync::Notify>,
...
impl ManagedTorrent {
    ...
    /// LOCAL PATCH: where this torrent's files live. `shared.options` is
    /// `pub(crate)`, so there is otherwise no way to implement "open containing
    /// folder" from outside the crate.
    ...
    /// LOCAL PATCH: drop back to Initializing so the next start() re-hashes
    /// every piece against what's on disk. Upstream only reaches this path
    /// after an error; there's no way to ask for a recheck deliberately.
    ///
    /// `previously_errored: true` is what makes it a *force* recheck — it
    /// clears the stored fastresume bitfield, so `check()` can't skip hashing.
    pub fn reset_to_initializing(self: &Arc<Self>) -> anyhow::Result<()> {
        let metadata = self
            .metadata
            .load_full()
            .context("torrent metadata isn't resolved yet")?;

        {
            let mut g = self.locked.write();

            // Tear the running state down first: the storage is recreated
            // below, and Windows won't reopen files the old state still holds.
            match g.state.take() {
                ManagedTorrentState::Live(live) => {
