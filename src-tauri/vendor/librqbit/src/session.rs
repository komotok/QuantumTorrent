...
pub struct SessionOptions {
    ...
    /// LOCAL PATCH: pin every socket to one local address (i.e. bind to a
    /// single network interface, such as a VPN tunnel). `None` = upstream
    /// behaviour, all interfaces.
    ///
    /// This covers all five paths that would otherwise leak the default
    /// interface: the inbound TCP listener, outbound peer connections, the DHT
    /// UDP socket, HTTP tracker requests, and UDP tracker requests.
    pub bind_ip: Option<std::net::IpAddr>,

    /// LOCAL PATCH: User-Agent for HTTP tracker announces. Upstream leaves the
    /// reqwest default ("reqwest/x.y"), which identifies the HTTP library
...
/// LOCAL PATCH: `bind_ip` replaces the hardcoded `0.0.0.0`, so the inbound
/// listener can be pinned to a single interface.
async fn create_tcp_listener(
    ...
    for port in port_range.clone() {
        match TcpListener::bind((ip, port)).await {
            ...
            Err(e) => {
                debug!("error listening on {ip}:{port}: {e:#}")
            }
        }
    }
    bail!("no free TCP ports in range {port_range:?} on {ip}");
}
...
impl Session {
        ...
        async move {
            ...
            let (tcp_listener, tcp_listen_port) =
                if let Some(port_range) = opts.listen_port_range.clone() {
                    let (l, p) = create_tcp_listener(port_range, opts.bind_ip)
                        .await
                    ...
                    info!("Listening on {bound}:{p} for incoming peer connections");
                    (Some(l), Some(p))
                } else {
                    (None, None)
                };
            ...
            let dht = if opts.disable_dht {
                ...
                // LOCAL PATCH: when bind_ip is set the DHT socket must be
                // pinned too, otherwise DHT traffic still egresses the default
                // interface and leaks the real IP. PersistentDht::create takes
                // no listen address (PersistentDhtConfig only carries
                // dump_interval/config_filename), so binding costs us DHT
                // routing-table persistence — bootstrap is slower on restart,
                // which is a fair trade for not leaking.
                let dht = if opts.disable_dht_persistence || opts.bind_ip.is_some() {
            ...
            let reqwest_client = {
                ...
                // LOCAL PATCH: pin HTTP tracker announces to the bound
                // interface. Without this the tracker sees the default
                // interface's address.
                let builder = match opts.bind_ip {
                    Some(ip) => builder.local_address(ip),
                    None => builder,
                };

                // LOCAL PATCH: identify as the app, not as reqwest.
                let builder = match opts.user_agent.as_deref() {
            ...
            // LOCAL PATCH: bind_ip pins outbound peer connections' source
            // address.
            let stream_connector =
                Arc::new(StreamConnector::new(proxy_config, opts.bind_ip));
            ...
            // LOCAL PATCH: bind_ip pins the UDP tracker socket.
            let udp_tracker_client = UdpTrackerClient::new(token.clone(), opts.bind_ip)
                .await
        ...
        loop {
            tokio::select! {
                ...
                Some(Ok((live, checked))) = futs.next(), if !futs.is_empty() => {
                    match live.add_incoming_peer(checked) {
                        Ok(()) => {
                            // LOCAL PATCH: count successful inbound peers so callers
                            // can distinguish "reachable" from "firewalled".
                        ...
                        Err(e) => warn!("error handing over incoming connection: {e:#}"),
                    }
                },
            }
        }
    ...
    async fn add_torrent_internal(
        ...
        let make_peer_rx = || {
            self.make_peer_rx(
                ...
                private,
                // No ManagedTorrent exists yet during magnet resolution, so
                // this one is never signalled - it just satisfies the API.
        ...
        let (managed_torrent, metadata) = {
            ...
            let minfo = Arc::new(ManagedTorrentShared {
                id,
                reannounce: Arc::new(tokio::sync::Notify::new()),
                span,
                info_hash,
    ...
    /// LOCAL PATCH: force a full re-verification of the torrent's data against
    /// disk, then resume. Every other client offers this; upstream has no
    /// public equivalent.
    /// LOCAL PATCH: ask every tracker loop for this torrent to announce now
    /// instead of waiting out its interval.
    pub fn force_reannounce(&self, handle: &ManagedTorrentHandle) {
        handle.shared().reannounce.notify_waiters();
    }
