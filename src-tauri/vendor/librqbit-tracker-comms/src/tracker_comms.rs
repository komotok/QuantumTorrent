...
pub struct TrackerComms {
    info_hash: Id20,
    /// LOCAL PATCH: notified to cut a sleep short and announce immediately.
...
impl TrackerComms {
    /// LOCAL PATCH: sleep until the interval elapses, or until someone asks for
    /// an immediate reannounce - whichever comes first. Upstream sleeps
    /// unconditionally, so there is no way to poke a tracker on demand.
    async fn sleep_or_reannounce(&self, dur: Duration) {
        tokio::select! {
            _ = tokio::time::sleep(dur) => {}
            _ = self.reannounce.notified() => {
                debug!("reannounce requested, waking tracker loop early");
            }
        }
    }
        ...
        loop {
            ...
            match self.tracker_one_request_http(tracker_url.clone()).await {
                Ok(interval) => {
                    ...
                    self.sleep_or_reannounce(interval).await;
                }
                Err(e) => {
                    debug!("error calling the tracker {}: {:#}", tracker_url, e);
                    self.sleep_or_reannounce(Duration::from_secs(60)).await;
                }
            };
        }
    }
    ...
    async fn task_single_tracker_monitor_udp(
        ...
        loop {
            if let Some(i) = sleep_interval {
                ...
                self.sleep_or_reannounce(i).await;
            }
