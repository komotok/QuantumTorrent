...
impl UdpTrackerClient {
    /// LOCAL PATCH: `bind_ip` pins the UDP tracker socket to one local address.
    /// Upstream hardcodes `0.0.0.0:0`, which announces the machine's default
    /// interface to every tracker — the exact leak that interface binding is
    /// meant to prevent. `None` keeps upstream behaviour (all interfaces).
    pub async fn new(
        cancel_token: CancellationToken,
        bind_ip: Option<IpAddr>,
    ) -> anyhow::Result<Self> {
        let bind_addr = SocketAddr::new(bind_ip.unwrap_or(IpAddr::V4(Ipv4Addr::UNSPECIFIED)), 0);
        let sock = tokio::net::UdpSocket::bind(bind_addr)
            .await
            .with_context(|| format!("error binding UDP for tracker on {bind_addr}"))?;
        let client = Self {
